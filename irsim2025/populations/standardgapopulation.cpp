...
void CStandardGAPopulation::CreateNextGeneration()
{
  ...
  //printf("Individual: %d -- SortFitness: %2f\n", j, (*m_vecIndividuals)[j]->GetFitness());
  /* DEBUG */
  ...
  for (i = 0; i < m_unNumberOfElites; i++)
  {
    ...
    //printf("ELITE: %2f %2f\n", (*m_vecIndividuals)[nIndex]->GetFitness(),(*m_vecIndividuals)[nIndex]->GetFitness());
  }
  ...
  */
  ...
  while (unIndividualsToCreate > 0)
  {
    ...
    /* NEED TO BE TESTED TO AVOID FATHER == MOTHER */
    //int nMotherIndex=0;
    //while ( !(fSelectionValue < pfOrderedFitness[nMotherIndex]) ) {
      //nMotherIndex++;
    //}
    //CIndividual* pcMother = (*m_vecIndividuals)[nMotherIndex];
    ////printf("Mother: %2f, %d -- CHR %2f %2f\n", fSelectionValue, nMotherIndex, pcMother->GetChromosome()[0], pcMother->GetChromosome()[1]);
    ...
    //{
      ...
      //while(!(fSelectionValue < pfOrderedFitness[nFatherIndex])){
        //nFatherIndex++;
      //}
      ////printf("MotherIndex: %d -- FatherIndex: %d\n", nMotherIndex, nFatherIndex);
    //}while(nFatherIndex == nMotherIndex);
    //
    //CIndividual* pcFather = (*m_vecIndividuals)[nFatherIndex];
    /* NEED TO BE TESTED TO AVOID FATHER == MOTHER */
    ...
    if(Random2::nextDouble() < m_fCrossoverRate){
      unCrossoverIndex=Random2::nextInt(m_unCrossoverDistance,m_unChromosomeLength); //nextInt(a,b) uniform int in [a,b-1]
    }
